...
fig, ax = plt.subplots()
plot_classifier(preds, ax, decision_boundary_xs=xs, decision_boundary_ys=ys)
...
#def calculate_quadratic_decision_boundary(classifier):
#  x1s = np.linspace(-1, 1, 1000)
#  y = x**2 + 2*x + 2

#xs, ys = calculate_quadratic_decision_boundary(logistic_classifier)
preds = logistic_classifier.predict(X)
fig, ax = plt.subplots()
...
most_frequent_classifier.fit(X, Y)

preds = most_frequent_classifier.predict(X)
acc = accuracy_score(Y, preds)

print("Most Frequent Train accuracy: {}".format(acc))
print(classification_report(Y, preds))
print()
plt.show()
